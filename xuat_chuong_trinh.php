...
$data = $db->withTotalCount()->get('chuong_trinh_dao_tao');
...
$filename = 'export_chuong_trinh_dao_tao.csv';

$num_queries = ceil($total_count / $chunk_size);

for ($i = 0; $i < $num_queries; $i++) {
    $rows = $db->get('chuong_trinh_dao_tao', array($offset, $chunk_size), $select);
    $offset += $chunk_size;
    foreach ($rows as $row) {
